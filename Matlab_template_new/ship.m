...
Ja = 0;                 % Ja = Va/(n*D)  advance number
PD = 1.5;               % pitch diameter ratio
AEAO = 0.65;            % blade area ration
...
[K_T, K_Q] = wageningen(Ja,PD,AEAO,z);
T = rho * Dia^4 * K_T * abs(n) * n; 
Q = rho * Dia^5 * K_Q * abs(n) * n; 

thr = T;    % thrust command (N)
...
Km = 0.6; % 0.6/s
Tm = 10; % 10s
df = 0.05; %deduction factor

Td = U_ref*Xu/(df-1);
n_c = sign(Td)*sqrt(abs(Td)/(rho*Dia^4*K_T));
Qc = rho * Dia^5 * K_Q * abs(n_c) * n_c; 
Y = Qc/Km;
Qm_dot = (Km*Y-Qm)/Tm; 
n_dot = (Qm-Q)/Im;
